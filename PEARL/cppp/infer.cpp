...
void Infer::run(string docs_pt, string model_dir, int idx) {
  load_para(model_dir, idx);
  
  cout << "Infer p(z|d) for docs in: " << docs_pt << endl;
  ifstream rf(docs_pt.c_str());
  assert(rf);

  string pt = model_dir + "k" + str_util::itos(K) + "." + str_util::itos(idx) + ".pz_d";	// k20.pz_d
...
void Infer::load_para(string model_dir, int idx) {
  string pt = model_dir + "k" + str_util::itos(K) + "." + str_util::itos(idx) + ".pz";	// k20.pz
  cout << "load p(z):" << pt <<endl;
  pz.loadFile(pt);
  assert(abs(pz.sum() - 1) < 1e-4);
 
  string pt2 = model_dir + "k" + str_util::itos(K) + "." + str_util::itos(idx) + ".pw_z";	// k20.pw_z
