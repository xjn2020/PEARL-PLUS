...
print(args)
model_base = '/workspace/OLineModel'
...
#     s = 'info'
# else:
#     s = 'info_' + args.model_name + '_' + args.iter + '_' + str(args.K) + '_' + str(args.gen_len) + '_' + str(args.part_of_dataset)
if args.iter is None:
...
with open(os.path.join('.', 'data', dataset_name, 'dataset.txt' if args.screen else 'dataset_pearl.txt'), mode='r', encoding='utf-8') as f:
...
# with open(os.path.join('.', 'data', dataset_name, 'top_k_similar_words.json'), 'r') as f:
...
# with open(os.path.join('.', 'data', dataset_name, f'df_{args.model_name}_gen_{dataset_name}.json'), 'w') as f:
if args.iter == None:
...
# model.save_pretrained_merged(f"models_merge/{dataset}/llama", tokenizer, save_method="merged_16bit", )
